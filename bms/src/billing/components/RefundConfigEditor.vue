...
const refundFeeLabel = name => name === '代收货款手续费' ? '代收服务费' : name
const deductFees = computed(() => feeRows.value
  .filter(item => item.type !== '非费项' && !excludedFeeCodes.has(item.code))
  .map(item => ({ label: refundFeeLabel(item.name), value: item.code })))
...
function validate() {
  ...
  for (let i = 0; i < form.currencyRules.length; i += 1) {
    ...
    if (!row.fallback && !row.sourceCurrency) return warn(`币种账户矩阵第${i + 1}行请选择返款原始币种`)
    if (!row.fallback && used.has(row.sourceCurrency)) return warn(`币种账户矩阵第${i + 1}行原始币种重复`)
    if (!row.settlementCurrency || row.settlementCurrency === 'SOURCE_CURRENCY') return warn(`币种账户矩阵第${i + 1}行请选择明确的返款结算币种`)
...
<template>
  <div class="config-editor refund-editor">
    <section class="rule-card">
      <header class="rule-head"><strong>COD返款条款</strong><el-switch v-model="form.enabled" /></header>
      <div class="config-no-bar"><span>配置版本编号</span><b>{{ config.no === '新配置' ? `${previewNo}-V1` : `${previewNo}-${config.publishVersion || config.version}` }}</b><small>{{ config.no === '新配置' ? '首次发布生成配置编号和 V1' : '发布后形成新版本，配置编号保持不变' }}</small></div>
