...
| GJ-026 | 核销 | `/finance/writeoffs`；`writeoffs.js` | 收/付各三页签、来源/列表/明细字段、查询/CSV、非样本已审申请投影、账号及金额试算；独立三状态样本可查看。209～210及039/105/106未决，保存、提交、打回、删除和客户额度回写阻断 |
| GJ-027 | 报销 | `/finance/reimbursements`；`reimbursements.js` | 报销/借支/退款源单、资产明细、付款/核销记录与完整录入预览；同单位同组织同币种、借支=报销+退款试算。源余额变化、状态维护、钉钉/金蝶及211～213未决路径阻断 |
| GJ-028 | 销项发票 | `/finance/invoices`；`invoices.js`、`invoiceActions.js` | 四页签、已确认应收来源、票面/主体/开票行预览、隔离样本CNY等额补录与一次性关联、合成信息CSV。申请保存/开票/删除、外部税务回传、正式发票文件和邮件受040～042、214～215限制 |
| GJ-029 | 对账单模板 | `/finance/statement-templates`、对账详情入口 | 13套字段schema、动态费用列、已知来源和逐源费用CSV预览；不建立可维护模板业务实体。标准模板选择、行归集、正式文件与邮件受196、216限制 |
| GJ-030 | 消息 | `/foundation/messages`、工作台、共享业务事件 | 保留既有合作方/空运通知；统一读取成本、申请、调度邮件及完整内容，接收人过滤与目标权限分离。9点车辆、18点未调度/合作方待审批只读预览；无真实投递，工作日阈值、航班预警、核销/开票回传及部分上游事件未接 |
| GJ-031 | 报表 | `/foundation/reports`；`reports.js` | 24个分角色入口，已定义查询及结果列；仓储/陆运当前源记录和航司建单月预览；财务与司机/供应商部分报表为隔离合成样表。电商三表字段未定义；审批生成、正式快照/导出及218～219未决指标受限 |
...
| --- | --- | --- |
| GJ-023-01 | §1.1.3、§1.2.1～1.2.7、§1.4.4；`FinanceCostsView`、`OrderCostFields`、`orderCosts` | 6条件已完成订单入口；应收应付各默认5行并可展开；完整字段、逐行草稿、编辑/删除、保存与提交分离、状态锁定、币种联动、参考价、溢价警告、20MB附件。新增应收及已知应付编辑/重提浏览器通过；新增应付及更换单位受186阻断 |
