...
| GJ-F08 / 税金与财务 | 已完成合成订单→成本保存→提交→客服主管审批→财务会计审批→待对账→组单保存→确认；应付既有已审成本→对账保存并确认；已审原行→调整单→业务审批及拒绝回流 | 押金/税金属性、应付新增、调整单最终生效；对账删除调整回流、标准模板/邮件；已确认对账→申请→核销→额度/余额回写；真实金蝶同步 |
...
- 当前逐篇游标：第024篇对账的上述确定路径已实现并核对；194～200受限路径不计为完成。下一篇为第025篇收付款申请；第009～023篇既有未决/未实现分支保留。
