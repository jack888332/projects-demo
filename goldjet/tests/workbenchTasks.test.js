...
describe('第002篇工作台：统计、分页和真实入口', () => {
  ...
  it('未实现快捷入口禁用且没有虚假跳转', () => {
    ...
    expect(service.find(item => item.label === '自建仓库订单')).toMatchObject({ disabled: false, target: { path: '/fulfillment/air-orders', query: { action: 'createWarehouse' } } })
    expect(getWorkbenchQuickLinks('warehouseSupervisor').find(item => item.label === '仓库月报')).toMatchObject({ disabled: false, target: { path: '/fulfillment/warehouse-orders', query: { tab: 'monthly' } } })
    expect(getWorkbenchQuickLinks('warehouseService').some(item => item.label === '仓库月报')).toBe(false)
