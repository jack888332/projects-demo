    ...
    <template v-else>
      ...
      <el-form :model="draft" label-position="top" @submit.prevent="save(false)">
        <section class="bill-section"><h2>提单标识与收发货人</h2><div class="bill-grid">
          ...
          <el-form-item v-for="party in [{ key: 'shipper', label: '发货人' }, { key: 'consignee', label: '收货人' }]" :key="party.key" :label="party.label" class="wide"><div class="contact-field"><el-input :model-value="draft[party.key].printText" readonly type="textarea" :rows="3" :aria-label="party.label" /><el-button :disabled="!editable" @click="openContact(party.key)">编辑{{ party.label }}</el-button></div></el-form-item>
        </div></section>
        ...
        <div class="edit-actions"><span v-if="dirty" class="field-note">有未暂存的修改</span><el-button @click="openOutput('托运书预览')">托运书预览</el-button><el-button @click="openOutput('预览')">预览</el-button><el-button @click="openOutput('下载')">下载</el-button><el-button :disabled="!editable || Boolean(Object.keys(errors).length)" @click="save(false)">暂存</el-button><el-button type="primary" :disabled="!editable || Boolean(Object.keys(errors).length)" @click="save(true)">提交</el-button></div>
      </el-form>
      <section class="bill-section"><div class="section-heading"><h2>航司发送</h2><el-button type="primary" :disabled="!canSend || !selection.length" @click="sendVisible = true">批量发送运单</el-button></div><p v-if="dirty" class="field-note">请先暂存当前修改，再发送运单。</p>
