      ...
      <el-form :model="draft" label-position="top" class="supplement-form" @submit.prevent="submit">
        <section class="supplement-section" aria-labelledby="supplement-info-title">
          ...
          <div class="supplement-grid">
            ...
            <el-form-item label="代码" :error="errors.code"><div class="code-field"><el-select v-model="draft.code" clearable :disabled="!canEditCode" aria-label="代码"><el-option value="EAP" /><el-option value="EAW" /></el-select><el-button v-business-write="'airOrders'" v-if="restriction && canEditCode" :disabled="busy || Boolean(errors.code)" @click="saveCode">保存代码</el-button></div></el-form-item>
        ...
        <section class="supplement-section" aria-labelledby="supplement-cargo-title">
          ...
          <template v-if="hasHouseBills">
            <div class="section-actions"><h3>分单</h3><div><el-button v-business-write="'airOrders'" :icon="Download" :disabled="Boolean(houseManagementRestriction) || busy" @click="openImport">引入子订单</el-button><el-button v-business-write="'airOrders'" :icon="Plus" :disabled="Boolean(houseManagementRestriction) || busy" @click="openHouse()">分单</el-button></div></div>
            ...
            <DataTableFrame :rows="children" :page-size="10" :page-sizes="[10]"><template #default="{ rows }"><el-table :data="rows" row-key="id" aria-label="主单下的分单" empty-text="尚未生成分单">
              ...
              <el-table-column label="操作" fixed="right" width="200"><template #default="{ row }"><el-button link type="primary" @click="openHouse(row)">{{ row.orderStatus === '子订单暂存' && !houseManagementRestriction ? '编辑' : '详情' }}</el-button><el-button v-business-write="'airOrders'" link type="primary" :disabled="Boolean(houseManagementRestriction) || row.orderStatus === '已取消' || busy" @click="childAction('move', row)">移单</el-button><el-button v-business-write="'airOrders'" link type="danger" :disabled="Boolean(houseManagementRestriction) || row.orderStatus !== '子订单暂存' || busy" @click="childAction('cancel', row)">取消</el-button></template></el-table-column>
        ...
        <div class="supplement-actions"><el-button :disabled="busy" @click="back">取消</el-button><el-button v-business-write="'airOrders'" v-if="canSubmit" type="primary" :loading="busy" :disabled="Object.keys(errors).length > 0" @click="submit">提交补录</el-button><el-button v-business-write="'airOrders'" v-else-if="!houseManagementRestriction && hasHouseBills" type="primary" :icon="Plus" @click="openHouse()">分单</el-button></div>
      </el-form>
