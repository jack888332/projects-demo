...
const orderCosts = computed(() => (typeof canReadModule !== 'function' || canReadModule('costs')) ? state.costs.filter(cost => cost.groundOrderId === selected.value?.id || bills.value.some(bill => bill.id === cost.waybillId)) : [])
