...
const allowed = computed(() => canManageAirClearances(unref(clearanceSession)))
  ...
  <div class="module-view clearance-view">
    ...
    <el-dialog v-model="detailVisible" title="清关派送详情" width="min(1050px, 96vw)" :close-on-click-modal="false" destroy-on-close>
      <div v-if="detail && allowed" class="clearance-dialog-body">
        ...
        <section><h3>接单记录</h3><dl v-if="detail.acceptance" class="clearance-grid"><div><dt>接单人</dt><dd>{{ detail.acceptance.actor }}</dd></div><div><dt>接单时间</dt><dd>{{ detail.acceptance.time }}</dd></div></dl><template v-else><p v-if="detail.acceptanceBlockReason" class="muted">{{ detail.acceptanceBlockReason }}</p><template v-else-if="accepting"><p>确认由 {{ unref(clearanceSession).name }} 接单？仅记录接单信息，服务状态仍以清关回执为准。</p><div class="clearance-actions"><el-button @click="accepting = false">取消</el-button><el-button type="primary" @click="confirmAcceptance">确认接单</el-button></div></template><el-button v-else type="primary" @click="prepareAcceptance">接单</el-button></template></section>
